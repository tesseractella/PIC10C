...
*/
int insertion_test(std::string letters)
{
    int count = 0;
    ...
    do
    {
        ...
        for (int i = 0; i < s.length(); i++)
        {
            ...
            if (t.count(e) == 0)
            {
                std::cout << e << " not inserted" << std::endl;
                pass = false;
            }
        }
        if (pass)
        ...
        {
            std::cout << "Failing for letters " << letters << std::endl;
        }
...
*/
void get_nodes(Node* n, std::vector<Node*>& nodes)
{
    if (n == nullptr) { return; }
...
int check_red_black(Node* n, bool is_root, bool report_errors)
{
    ...
    if (nleft != nright)
    {
        if (report_errors)
        {
            std::cout << "Left and right children of " << n->data
                << " have different black depths" << std::endl;
        }
        return -1;
    }
    if (n->parent == nullptr)
    {
        if (!is_root)
        {
            if (report_errors)
            {
                std::cout << n->data << " is not root and has no parent" << std::endl;
            }
            return -1;
        }
        if (n->color != BLACK)
        {
            if (report_errors)
            {
                std::cout << "Root " << n->data << " is not black";
            }
            return -1;
        }
    }
    else
    {
        if (is_root)
        {
            if (report_errors)
            {
                std::cout << n->data << " is root and has a parent" << std::endl;
            }
            return -1;
        }
        if (n->color == RED && n->parent->color == RED)
        {
            if (report_errors)
            {
                std::cout << "Parent of red " << n->data << " is red" << std::endl;
            }
            return -1;
        }
    }
    if (n->left != nullptr && n->left->parent != n)
    {
        if (report_errors)
        {
            std::cout << "Left child of " << n->data
                << " has bad parent link" << std::endl;
        }
        return -1;
    }
    if (n->right != nullptr && n->right->parent != n)
    {
        if (report_errors)
        {
            std::cout << "Right child of " << n->data
                << " has bad parent link" << std::endl;
        }
        return -1;
    }
    if (n->color != RED && n->color != BLACK)
    {
        if (report_errors)
        {
            std::cout << n->data << " has color " << n->color;
        }
        return -1;
    }
...
bool check_red_black(RedBlackTree& t, bool report_errors)
{
    ...
    for (int i = 0; i < nodes.size() - 1; i++)
    {
        if (nodes[i]->data > nodes[i + 1]->data > 0)
        {
            if (report_errors)
            {
                std::cout << nodes[i]->data << " is larger than " <<
                    nodes[i + 1]->data << std::endl;
            }
            return false;
        }
    }
...
int removal_test(const RedBlackTree& t)
{
    ...
    for (int m = 0; m <= 1; m++)
    {
        ...
        for (int k = 0; k < std::pow(2, nodes_to_color); k++)
        {
            RedBlackTree rb;
            ...
            if (check_red_black(rb, false)) // Found a valid tree
            {
                ...
                for (int j = 0; j < filled_size; j++)
                {
                    ...
                    if (rb.count(s) == 0 && d != s)
                    {
                        std::cout << s << " deleted" << std::endl;
                        pass = false;
                    }
                }
                if (rb.count(d) > 0)
                {
                    std::cout << d << " not deleted" << std::endl;
                    pass = false;
                }
                if (pass)
                {
                ...
                {
                    std::cout << "Failing tree: " << std::endl;
...
int main()
{
	...
	t.erase("q");
	t.print();
    std::cout << check_red_black(t, true) << std::endl;
	t.insert("a");
	t.insert("b");
	t.insert("c");
	t.insert("d");
	t.insert("e");
	int passing = removal_test(t);
    std::cout << passing << " removal tests passed." << std::endl;
	passing = insertion_test("ABCDEFGHI");
    std::cout << passing << " insertion tests passed." << std::endl;
}
